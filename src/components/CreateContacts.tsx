...
function CreateContacts({onCreate}: CreateContactsProps) {
  ...
  const  submitHandler = async (e: React.FormEvent) => {
   ...
   const response = await  axios.post<IContacts>('http://localhost:3000/contacts', contactData)
  ...
  return (
    <form onSubmit={submitHandler} className='window'>
      <input type="text"  placeholder='Введите имя' value={name} onChange={changeName} className="input"/>
      <input type="text"  placeholder='Введите фамилию' value={secondName} onChange={changeSecondName} className="input"/>
      {error &&<p>{error}</p>}
