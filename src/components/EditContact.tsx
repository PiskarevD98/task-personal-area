...
function EditContact({contact, onEdit}: ContactsProps) {
  ...
  const  submitHandler = async (e: React.FormEvent) => {
    ...
    if(secondName.trim().length === 0 ) {
      ...
      const response = await  axios.put<IContacts>(`http://localhost:3000/contacts/${contactId}`, contactData)
  ...
  return (
    <form onSubmit={submitHandler} className=''>
      ...
      <input type="text" className="input"  value={secondName} onChange={changeSecondName}/>
      {error &&<p>{error}</p>}
